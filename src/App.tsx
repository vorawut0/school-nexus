...
import { recordUserPresence } from './services/telemetryService';
...
export default function App() {
  ...
  // Apply theme to DOM documentElement and body with smooth transition tracking
  useEffect(() => {
    try {
  ...
  const handleToggleTheme = (targetTheme?: 'light' | 'dark') => {
    ...
    // Add global theme transition class for smooth CSS interpolation
    if (typeof document !== 'undefined') {
      ...
      // Utilize Native Document View Transition API for hardware-accelerated cross-fade
  ...
  useEffect(() => {
    ...
    const handleAvatarChangedEvent = (e: any) => {
    ...
    window.addEventListener('sn_avatar_changed', handleAvatarChangedEvent);

    return () => {
      window.removeEventListener('online', handleOnline);
      window.removeEventListener('offline', handleOffline);
      window.removeEventListener('sn_system_full_reset', handleSystemResetEvent);
      window.removeEventListener('sn_avatar_changed', handleAvatarChangedEvent);
    };
  }, []);
    ...
    const unsubUserProfile = subscribeToUserProfile(user.id, (updatedProfile) => {
      if (updatedProfile) {
        setUser((prev) => {
          ...
          // Only update if avatar, cardTheme, or crucial fields changed to avoid unnecessary re-renders
          if (
            prev.avatar !== updatedProfile.avatar ||
            prev.cardTheme !== updatedProfile.cardTheme ||
  ...
  // Real-time Presence Heartbeat: record user activity every 30 seconds
  useEffect(() => {
    if (!user) return;
  ...
  const handleSignOut = () => {
    try {
      localStorage.setItem('sn_logout_explicit', 'true');
      localStorage.removeItem('sn_active_user');
      localStorage.removeItem('sn_user_profile');
      localStorage.removeItem('sn_last_active_user');
  ...
  return (
    <div className="min-h-screen w-screen max-w-full overflow-x-hidden bg-slate-50 text-slate-900 flex flex-col font-['Noto_Sans_Thai',sans-serif] selection:bg-blue-700 selection:text-white">
      {/* Fixed Top Header */}
      <Header
      ...
      {/* Main View Router */}
      <main className="flex-1 w-full max-w-full overflow-x-hidden flex flex-col px-0 pb-20 sm:pb-24">
      ...
      <button
        ...
        className={`fixed right-4 bottom-20 sm:bottom-22 z-40 px-4 py-3 text-white rounded-full shadow-lg hover:shadow-xl hover:scale-105 active:scale-95 transition-all flex items-center gap-2 font-bold text-[13px] border border-white/20 cursor-pointer group ${
          user.role === 'teacher'
      ...
      <DigitalIdModal
        ...
        onUpdateTheme={(newTheme) => {
          setUser((prev) => {
            if (!prev) return prev;
            const updated = { ...prev, cardTheme: newTheme };
            savePersistedCardTheme(updated, newTheme);
      ...
      <EditProfileModal
        ...
        onSaveProfile={(updatedProfile) => {
          if (updatedProfile.cardTheme) {
            savePersistedCardTheme(updatedProfile, updatedProfile.cardTheme);
          }
