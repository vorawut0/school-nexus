...
export const TeacherDashboard: React.FC<TeacherDashboardProps> = ({
  ...
  const [dashboardFileViewer, setDashboardFileViewer] = useState<StudentWorkViewerData | null>(null);
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 py-6 space-y-6 animate-fadeIn pb-24">
      ...
      {toastMsg && (
        <div className="fixed top-4 right-3 sm:right-6 z-[90] bg-slate-900 text-white px-4 py-3 rounded-2xl shadow-2xl border border-blue-500/40 flex items-center gap-3 text-xs font-semibold animate-slideInRightToast max-w-[calc(100vw-24px)] sm:max-w-md pointer-events-auto">
          ...
          {submissionsQueue.map((sub) => (
            ...
            >
              <div>
                ...
                {/* Attached file (Interactive button) */}
                <button
                  type="button"
                  onClick={() => setDashboardFileViewer(sub)}
                  className="mt-2 w-full px-2.5 py-1.5 rounded-xl bg-blue-50/70 hover:bg-blue-100/80 border border-blue-200/80 text-[11px] text-blue-800 flex items-center justify-between gap-1.5 transition-all cursor-pointer group"
                  title="คลิกเพื่อเปิดดูไฟล์งานของนักเรียน"
                >
                  <div className="flex items-center gap-1.5 truncate">
                    <span className="material-symbols-outlined text-[15px] text-blue-600">visibility</span>
                    <span className="truncate font-mono">{sub.fileAttachment}</span>
                  </div>
                  <span className="text-[10px] font-bold text-blue-700 bg-blue-200/60 px-1.5 py-0.2 rounded shrink-0 group-hover:bg-blue-300/80">
                    เปิดดู
                  </span>
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 mt-1">
                <button
                  type="button"
                  onClick={() => setDashboardFileViewer(sub)}
                  className="py-2 rounded-xl font-bold text-xs flex items-center justify-center gap-1 bg-slate-100 hover:bg-slate-200 text-slate-700 cursor-pointer transition-all"
                >
                  <span className="material-symbols-outlined text-[15px] text-blue-600">preview</span>
                  <span>ดูไฟล์</span>
                </button>
                <button
                  onClick={() => {
                  ...
                  className={`py-2 rounded-xl font-bold text-xs flex items-center justify-center gap-1 cursor-pointer transition-all ${
                    sub.status === 'graded'
                ...
                >
                  ...
                  <span>{sub.status === 'graded' ? 'แก้คะแนน' : 'ให้คะแนน'}</span>
                </button>
              </div>
            </div>
          ))}
        ...
        >
          ...
          >
            ...
            <form onSubmit={handleSaveQuickGrade} className="p-5 space-y-4">
              ...
              {/* File Attachment & Inspector Link */}
              <div className="p-2.5 bg-blue-50/60 rounded-xl border border-blue-200 flex items-center justify-between gap-2">
                <div className="flex items-center gap-1.5 text-xs text-blue-900 font-mono truncate">
                  <span className="material-symbols-outlined text-[15px] text-blue-600">attach_file</span>
                  <span className="truncate">{quickScoreModal.fileAttachment}</span>
                </div>
                <button
                  type="button"
                  onClick={() => {
                    const toView = quickScoreModal;
                    setQuickScoreModal(null);
                    setDashboardFileViewer(toView);
                  }}
                  className="px-2.5 py-1 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-[11px] font-bold flex items-center gap-1 cursor-pointer shadow-xs shrink-0"
                >
                  <span className="material-symbols-outlined text-[13px]">visibility</span>
                  <span>เปิดดูไฟล์ & โค้ด</span>
                </button>
              </div>
      ...
      {/* Student Work Viewer Modal */}
      {dashboardFileViewer && (
        <StudentWorkViewerModal
          data={dashboardFileViewer}
          onClose={() => setDashboardFileViewer(null)}
          onGradeSubmit={(id, score, fb) => {
            setSubmissionsQueue((prev) =>
              prev.map((s) => (s.id === id ? { ...s, grade: score, status: 'graded' } : s))
            );
            showToast(`บันทึกคะแนน ${score}/${dashboardFileViewer.maxScore} ให้ ${dashboardFileViewer.thaiName} สำเร็จแล้ว!`);
            setDashboardFileViewer(null);
          }}
        />
      )}
    </div>
