...
export const TeacherClassesView: React.FC<TeacherClassesViewProps> = ({
  ...
  const [selectedMaterialsClass, setSelectedMaterialsClass] = useState<ClassroomData | null>(null);
  const [selectedRosterClass, setSelectedRosterClass] = useState<ClassroomData | null>(null);
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-4 sm:p-6 pb-28 space-y-6">
        ...
        {filteredClasses.map((classroom) => (
          ...
          >
            ...
            <div className="flex items-center gap-2 pt-2 border-t border-slate-100">
              <button
                type="button"
                onClick={() => setSelectedMaterialsClass(classroom)}
                ...
                title={`ดูสื่อและแผนการสอนวิชา ${classroom.subjectCode}`}
              >
              ...
              <button
                type="button"
                onClick={() => setSelectedRosterClass(classroom)}
                ...
                title={`ดูรายชื่อนักเรียนห้อง ${classroom.thaiGrade}`}
              >
      ...
      {selectedRosterClass && (
        <StudentRosterModal
          classroom={selectedRosterClass}
          onClose={() => setSelectedRosterClass(null)}
        />
      )}

      {/* Class Materials & Lesson Plans Modal */}
      {selectedMaterialsClass && (
        <ClassMaterialsModal
          classroom={selectedMaterialsClass}
          onClose={() => setSelectedMaterialsClass(null)}
        />
      )}
    </div>
  );
