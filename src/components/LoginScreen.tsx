...
export const LoginScreen: React.FC<LoginScreenProps> = ({
  ...
  // Stored registered accounts on this device
  const [storedAccounts, setStoredAccounts] = useState<StoredAccountRecord[]>(() => {
    return getStoredAccounts();
  });
  const [showManualForm, setShowManualForm] = useState<boolean>(false);

  // Sync stored accounts
  const refreshStoredAccounts = () => {
  ...
  const handleQuickLogin = async (acc: StoredAccountRecord) => {
    ...
    // Try signing in
    const pwToTry = acc.password || 'password';
    const res = await signInUser(acc.studentId || acc.email, pwToTry, acc.role);
    setIsLoading(false);

    if (res.success && res.user) {
      showToast(`เข้าสู่ระบบสำเร็จ: ${res.user.thaiName || res.user.name}`, 'success');
      onLoginSuccess(res.user);
    } else {
      setShowManualForm(true);
      showToast('กรุณากรอกรหัสผ่านเพื่อเข้าสู่ระบบ', 'error');
      const pwInput = document.getElementById('login-password-input') as HTMLInputElement;
      if (pwInput) {
        pwInput.focus();
      }
    }
  ...
  const handleRemoveSavedAccount = (e: React.MouseEvent, idOrEmail: string) => {
    e.stopPropagation();
    removeStoredAccount(idOrEmail);
    refreshStoredAccounts();
    showToast('ลบบัญชีออกจากเครื่องนี้เรียบร้อยแล้ว', 'success');
  };
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    const emailValidation = validateRealEmail(regEmail);
    if (!emailValidation.isValid) {
      showToast(emailValidation.error || 'อีเมลที่ใช้ต้องเป็นอีเมลจริงที่มีอยู่จริง เช่น yourname@gmail.com', 'error');
      return;
    }
  ...
  const handleResetPassword = async () => {
    const emailValidation = validateRealEmail(resetEmail);
    if (!emailValidation.isValid) {
      showToast(emailValidation.error || 'กรุณากรอกอีเมลจริงที่ถูกต้อง เช่น yourname@gmail.com', 'error');
      return;
    }
  ...
  return (
    <div className="relative min-h-screen w-full bg-[#f9f9ff] text-[#121b2e] flex flex-col justify-between items-center px-4 pt-6 sm:pt-8 pb-8 font-['Noto_Sans_Thai',sans-serif]">
      ...
      {toastMessage && (
        <div
          className={`fixed top-4 right-3 sm:right-6 z-50 px-4 py-3 rounded-2xl shadow-xl text-xs sm:text-sm font-semibold flex items-center gap-2 border transition-all animate-slideInRightToast max-w-[calc(100vw-24px)] sm:max-w-md ${
        ...
        {authMode === 'signin' ? (
          <div className="w-full flex flex-col gap-3.5">
            ...
            {storedAccounts && storedAccounts.length > 0 && (
              <div className="w-full bg-white/95 rounded-2xl p-3.5 border border-blue-100 shadow-xs">
                <div className="flex items-center justify-between mb-2.5 px-0.5">
                  <div className="flex items-center gap-1.5 text-xs font-bold text-[#121b2e]">
                    <span className="material-symbols-outlined text-[18px] text-[#1550d3]">badge</span>
                    <span>บัญชีที่เคยเข้าใช้งานในเครื่องนี้ ({storedAccounts.length})</span>
                  </div>
                  <span className="text-[11px] text-blue-600 font-semibold">แตะเพื่อเข้าสู่ระบบ</span>
                </div>

                <div className="flex flex-col gap-2 max-h-56 overflow-y-auto pr-1">
                  {storedAccounts.map((acc, idx) => {
                    const isCurrentRole = acc.role === selectedRole;
                    ...
                    return (
                      <div
                        key={`${acc.id || acc.studentId || idx}`}
                        onClick={() => handleQuickLogin(acc)}
                        className={`p-2.5 rounded-xl border flex items-center justify-between gap-2.5 cursor-pointer transition-all hover:shadow-md ${
                          isCurrentRole
                            ? 'bg-[#f0f4ff] border-blue-300 ring-1 ring-blue-400/30'
                      ...
                      >
                        <div className="flex items-center gap-2.5 min-w-0">
                          <img
                            ...
                            onError={(e) => {
                              e.currentTarget.src = ASSETS.headerAvatar;
                            }}
                          />
                          <div className="min-w-0">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              <span className="font-bold text-[13px] text-[#121b2e] truncate">
                                {acc.thaiName || acc.name}
                              </span>
                            ...
                            <div className="text-[11px] text-[#737686] truncate">
                              {acc.studentId ? `ID: ${acc.studentId}` : acc.email}
                              {acc.user?.grade ? ` • ${acc.user.grade}` : ''}
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-1.5 shrink-0">
                          <button
                            type="button"
                            disabled={isLoading}
                            className="px-3 py-1.5 bg-[#1550d3] hover:bg-[#103eb0] text-white rounded-lg text-xs font-bold shadow-xs cursor-pointer flex items-center gap-1"
                          >
                          ...
                          <button
                            type="button"
                            onClick={(e) => handleRemoveSavedAccount(e, acc.id || acc.studentId || acc.email)}
            ...
            <div className="flex items-center gap-2 my-1 px-1">
              ...
              <span className="text-[11px] font-semibold text-[#737686]">
                {storedAccounts && storedAccounts.length > 0 ? 'หรือกรอกรหัสประจำตัว/อีเมล' : 'กรอกข้อมูลเข้าสู่ระบบ'}
              </span>
              <div className="flex-1 h-px bg-slate-300/60" />
            </div>
          ...
          <form onSubmit={handleSignUp} className="w-full flex flex-col gap-3.5">
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="text-[11px] font-bold text-[#434654]">
                  อีเมลสำหรับเข้าสู่ระบบ (ต้องเป็นอีเมลจริง) *
                </label>
                <span className="text-[10px] text-blue-600 font-medium">
                  เช่น Gmail, Outlook, อีเมลโรงเรียน
                </span>
              </div>
              <input
                type="email"
                ...
                placeholder="yourname@gmail.com หรือ email@school.ac.th"
                className="w-full bg-white text-[#121b2e] placeholder:text-[#737686] text-[13px] rounded-xl py-2.5 px-3.5 shadow-xs border border-slate-200 focus:border-[#1550d3] focus:outline-none"
                required
              />
              <span className="text-[10.5px] text-slate-500 block mt-1">
                * ระบบจะใช้ที่อยู่อีเมลนี้ในการยืนยันตัวตน กู้คืนรหัสผ่าน และรับการแจ้งเตือน
              </span>
            </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 sm:p-7 max-w-md w-full shadow-2xl border border-slate-100 animate-scaleIn">
            ...
            {resetSent ? (
              ...
              <div>
                <p className="text-xs text-[#434654] mb-4">
                  กรอกอีเมลที่คุณใช้ลงทะเบียนเพื่อรับลิงก์สำหรับตั้งรหัสผ่านใหม่ (เช่น yourname@gmail.com)
                </p>
                <input
                  type="email"
                  value={resetEmail}
