...
interface ProfileViewProps {
  user: UserProfile;
  theme?: 'light' | 'dark';
  onToggleTheme?: (targetTheme?: 'light' | 'dark') => void;
  isOffline?: boolean;
      ...
      <div className="flex flex-col gap-5 sm:gap-8 max-w-full">
          ...
          {(() => {
            ...
            return (
              ...
              >
                ...
                >
                  <GuillochePatternSvg themeId={activeTheme} opacity={0.16} />
        ...
        <section className="bg-white rounded-2xl p-5 sm:p-6 shadow-sm border border-slate-200/80 flex flex-col gap-3.5">
          ...
          <div className="flex flex-col divide-y divide-slate-100 text-sm">
            {/* Theme Toggle (Light / Dark Mode) with Smooth Cross-Fade Preview */}
            <div className="py-3.5 flex flex-col gap-3">
              <div className="flex justify-between items-start">
                <div>
                  <div className="font-semibold text-[#121b2e] flex items-center gap-2">
                    <span className="material-symbols-outlined text-[18px] text-[#1550d3]">palette</span>
                    <span>ธีมการแสดงผล (Appearance)</span>
                    <span className={`text-[11px] px-2.5 py-0.5 rounded-full font-bold transition-all duration-300 ${
                      theme === 'dark'
                ...
                {/* Smooth Animated Switch with Cross-Fade Icons */}
                <button
                  type="button"
                  role="switch"
                  aria-checked={theme === 'dark'}
                  onClick={() => onToggleTheme?.()}
                  className={`relative inline-flex h-8 w-14 shrink-0 cursor-pointer rounded-full p-0.5 border-2 transition-all duration-400 ease-out focus:outline-none focus-visible:ring-2 focus-visible:ring-[#1550d3] active:scale-95 shadow-inner ${
                    theme === 'dark'
                      ? 'bg-slate-800 border-indigo-500/60 shadow-indigo-950/50'
                      : 'bg-amber-100 border-amber-400/80 shadow-amber-200/50'
                  }`}
                  title={theme === 'dark' ? 'กำลังใช้โหมดมืด (คลิกเพื่อเปลี่ยนเป็นโหมดสว่าง)' : 'กำลังใช้โหมดสว่าง (คลิกเพื่อเปลี่ยนเป็นโหมดมืด)'}
                >
                  <span
                    aria-hidden="true"
                    className={`relative pointer-events-none flex items-center justify-center h-6.5 w-6.5 transform rounded-full shadow-md transition-all duration-400 ease-out ${
                      theme === 'dark'
                        ? 'translate-x-6 bg-gradient-to-br from-indigo-900 to-slate-900 text-indigo-200 ring-1 ring-indigo-400/40'
                        : 'translate-x-0 bg-white text-amber-500 ring-1 ring-amber-300/60'
                    }`}
                  >
                    {/* Cross-fade Light Mode Icon */}
                    <span
                      className={`material-symbols-outlined text-[16px] absolute transition-all duration-400 ease-out ${
                        theme === 'dark'
              ...
              {/* Interactive Theme Selection Cards with Cross-Fade State */}
              <div className="grid grid-cols-2 gap-2.5 pt-1">
                <button
                  type="button"
                  onClick={() => onToggleTheme?.('light')}
                  className={`group relative p-3 rounded-xl border text-left transition-all duration-300 cursor-pointer overflow-hidden ${
                    theme === 'light'
                      ? 'border-[#1550d3] bg-blue-50/70 shadow-sm ring-2 ring-[#1550d3]/20'
                      : 'border-slate-200 hover:border-slate-300 bg-slate-50/50 hover:bg-slate-50 opacity-80 hover:opacity-100'
                  }`}
                >
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center gap-1.5 font-bold text-xs text-slate-800">
                      <span className="material-symbols-outlined text-[16px] text-amber-500 transition-transform duration-300 group-hover:rotate-45">
                        wb_sunny
                      </span>
                      <span>โหมดสว่าง</span>
                    </div>
                    {theme === 'light' && (
                      <span className="material-symbols-outlined text-[16px] text-[#1550d3] animate-scaleIn">
                        check_circle
                      </span>
                    )}
                  </div>
                  <div className="h-6 w-full rounded-lg bg-white border border-slate-200/90 flex items-center px-2 gap-1 shadow-xs transition-colors">
                    <div className="w-2.5 h-2.5 rounded-full bg-blue-500 shrink-0" />
                    <div className="w-10 h-1.5 rounded-full bg-slate-200" />
                    <div className="w-4 h-1.5 rounded-full bg-slate-100 ml-auto" />
                  </div>
                  <div className="text-[10px] text-slate-500 font-medium mt-1.5">
                    สบายตา ชัดเจนทุกมุมมอง
                  </div>
                </button>
                ...
                <button
                  ...
                  className={`group relative p-3 rounded-xl border text-left transition-all duration-300 cursor-pointer overflow-hidden ${
                    theme === 'dark'
                      ? 'border-indigo-400 bg-slate-900 text-white shadow-sm ring-2 ring-indigo-500/30'
                      : 'border-slate-200 hover:border-slate-300 bg-slate-50/50 hover:bg-slate-50 opacity-80 hover:opacity-100'
                  }`}
                >
                  <div className="flex items-center justify-between mb-1.5">
                    <div className={`flex items-center gap-1.5 font-bold text-xs ${
                      ...
                      <span className="material-symbols-outlined text-[16px] text-indigo-400 transition-transform duration-300 group-hover:-rotate-12">
                        bedtime
                      </span>
                      <span>โหมดมืด</span>
                    </div>
                    {theme === 'dark' && (
                      <span className="material-symbols-outlined text-[16px] text-indigo-400 animate-scaleIn">
                        check_circle
                      </span>
                    )}
                  </div>
                  <div className="h-6 w-full rounded-lg bg-slate-950 border border-slate-800 flex items-center px-2 gap-1 shadow-xs transition-colors">
                    <div className="w-2.5 h-2.5 rounded-full bg-indigo-500 shrink-0" />
                    <div className="w-10 h-1.5 rounded-full bg-slate-700" />
                    <div className="w-4 h-1.5 rounded-full bg-slate-800 ml-auto" />
                  </div>
                  <div className={`text-[10px] font-medium mt-1.5 ${
                    theme === 'dark' ? 'text-indigo-300/80' : 'text-slate-500'
                  }`}>
                    มืดสนิท ประหยัดพลังงาน
                  </div>
                </button>
              </div>
            </div>
          ...
          <div className="mt-2">
            <button
              type="button"
              onClick={onSignOut}
              className="w-full py-3 px-4 rounded-xl bg-red-50 text-[#ba1a1a] hover:bg-red-100 font-semibold text-sm transition-colors flex items-center justify-center gap-2 border border-red-200 cursor-pointer active:scale-98"
            >
