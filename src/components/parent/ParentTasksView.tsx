            ...
            >
              <div className="flex justify-between items-start gap-2">
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  ...
                  {as.lastSyncedAt ? (
                    <span
                      className="px-2 py-1 rounded-lg text-xs font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200 flex items-center gap-1 shadow-2xs"
                      title={`GitHub Synced: ซิงค์แล้วเมื่อ ${new Date(as.lastSyncedAt).toLocaleDateString('th-TH')}`}
                    >
                      <span className="material-symbols-outlined text-[14px] text-emerald-600 font-bold fill-1">check_circle</span>
                      <span className="text-[10px] hidden sm:inline">Synced</span>
                    </span>
                  ) : !as.githubRepoUrl || !as.githubRepoUrl.trim() ? (
                    <span
                      className="px-2 py-1 rounded-lg text-xs font-semibold bg-amber-50 text-amber-700 border border-amber-200 flex items-center gap-1 shadow-2xs"
                      title="GitHub Sync: ยังไม่ระบุ Repository URL"
                    >
                      <span className="material-symbols-outlined text-[14px] text-amber-600 font-bold fill-1">warning</span>
                      <span className="text-[10px] hidden sm:inline">No Repo</span>
                    </span>
                  ) : (
                    <span
                      className="px-2 py-1 rounded-lg text-xs font-semibold bg-blue-50 text-blue-700 border border-blue-200 flex items-center gap-1 shadow-2xs"
                      title={`GitHub Repo Linked: ${as.githubRepoUrl}`}
                    >
