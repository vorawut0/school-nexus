...
import { useLiveCampusTelemetry } from '../services/telemetryService';
...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  const liveTelemetry = useLiveCampusTelemetry();
  ...
  const [displayStudentCount, setDisplayStudentCount] = useState(0);

  // Animate counter to real student count
  useEffect(() => {
    const target = liveTelemetry.totalStudents;
    if (target === 0) {
      setDisplayStudentCount(0);
      return;
    }
    let start = 0;
    ...
    const timer = setInterval(() => {
      start += step;
      if (start >= target) {
        setDisplayStudentCount(target);
        clearInterval(timer);
      } else {
        setDisplayStudentCount(start);
      }
    }, 25);
    return () => clearInterval(timer);
  }, [liveTelemetry.totalStudents]);
      ...
      <div className="z-10 w-full flex flex-col gap-6 sm:gap-8">
        ...
        <section className="flex flex-col gap-2">
          <button
            ...
            className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-blue-50 hover:bg-blue-100/80 text-blue-800 w-fit border border-blue-200 shadow-2xs hover:shadow-xs transition-all duration-200 cursor-pointer active:scale-95 group"
            title="คลิกเพื่อเปิดสรุปประจำวัน & รับพลังบวก 🌟"
          >
            <span className="material-symbols-outlined text-[16px] text-blue-700 group-hover:rotate-12 transition-transform">wb_twilight</span>
            ...
            <span className="text-[10px] bg-blue-700 text-white px-2 py-0.5 rounded-full font-bold ml-0.5 group-hover:scale-105 transition-transform">
              สรุปประจำวัน ✨
            </span>
          </button>
          <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-slate-900 leading-snug tracking-tight">
            {user.thaiName.split(' ')[0]}, ยินดีต้อนรับกลับสู่ SCHOOL NEXUS
          </h1>
          <p className="text-slate-600 text-xs sm:text-sm md:text-[15px]">
        ...
        <section className="relative">
          <div className="relative bg-white rounded-[24px] p-5 sm:p-6 shadow-sm flex flex-col gap-5 border border-slate-200/80">
            {/* AI Card Header */}
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className={`w-12 h-12 rounded-2xl text-white flex items-center justify-center relative shadow-sm ${
                  user.role === 'teacher'
                    ? 'bg-blue-700 shadow-blue-900/20'
                    : user.role === 'admin'
                    ? 'bg-slate-900 shadow-slate-900/20'
                    : user.role === 'parent'
                    ? 'bg-amber-700 shadow-amber-900/20'
                    : 'bg-slate-900 shadow-slate-900/20'
                }`}>
                  ...
                  <div className="absolute top-0 right-0 w-3 h-3 bg-emerald-500 rounded-full border-2 border-white shadow-xs"></div>
                </div>
                <div>
                  <h2 className="font-bold text-[18px] text-slate-900 flex items-center gap-1.5">
                    {user.role === 'teacher'
              ...
              <div className="flex items-center gap-1.5">
                {onOpenAITutor && (
                  <button
                    ...
                    className="px-3.5 py-1.5 rounded-xl bg-slate-900 text-white hover:bg-slate-800 flex items-center gap-1.5 text-[12px] font-bold shadow-xs transition-all active:scale-95 cursor-pointer"
                ...
                <button
                  ...
                  className="w-10 h-10 rounded-xl bg-slate-100 text-slate-600 flex items-center justify-center hover:bg-slate-200 hover:text-slate-900 transition-colors cursor-pointer"
                  title="สรุปภาพรวม AI"
                >
            ...
            {/* AI Speech Bubble */}
            <div className="bg-slate-50 p-4 rounded-2xl relative border border-slate-200 transition-all">
              <div className="absolute -top-1.5 left-6 w-3 h-3 bg-slate-50 border-t border-l border-slate-200 rotate-45"></div>
              <p className="text-[15px] sm:text-[16px] text-slate-800 relative z-10 leading-relaxed">
                {aiThinking ? (
            ...
            <div className="grid grid-cols-2 gap-2.5 sm:gap-3">
              ...
              <button
                ...
                className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 hover:bg-slate-100 hover:shadow-2xs transition-all group border border-slate-200/80 text-left active:scale-98 cursor-pointer"
              >
                <div className="w-9 h-9 rounded-lg bg-white text-blue-700 flex items-center justify-center shadow-xs border border-slate-200/60 group-hover:scale-105 transition-transform">
                  <span className="material-symbols-outlined text-[20px]">menu_book</span>
                </div>
                <div className="flex flex-col">
                  <span className="font-semibold text-[13px] sm:text-[14px] text-slate-800 group-hover:text-blue-700 transition-colors">
                    {user.role === 'teacher'
                      ? 'ตารางสอนวันนี้'
                      : user.role === 'admin'
                      ? 'ตารางปฏิบัติงาน'
              ...
              <button
                ...
                className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 hover:bg-slate-100 hover:shadow-2xs transition-all group border border-slate-200/80 text-left active:scale-98 cursor-pointer"
              >
                <div className="w-9 h-9 rounded-lg bg-white text-rose-600 flex items-center justify-center shadow-xs border border-slate-200/60 group-hover:scale-105 transition-transform">
              ...
              <button
                ...
                className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 hover:bg-slate-100 hover:shadow-2xs transition-all group border border-slate-200/80 text-left active:scale-98 cursor-pointer"
              >
                <div className="w-9 h-9 rounded-lg bg-white text-emerald-700 flex items-center justify-center shadow-xs border border-slate-200/60 group-hover:scale-105 transition-transform">
                  <span className="material-symbols-outlined text-[20px]">
            ...
            >
              <input
                ...
                className="w-full h-13 pl-11 pr-13 rounded-2xl bg-slate-50 text-slate-900 text-[14px] placeholder:text-slate-400 border border-slate-200 focus:border-blue-600 focus:ring-2 focus:ring-blue-600/10 focus:outline-none transition-all"
              />
              ...
              <button
                type="submit"
                className="absolute right-1.5 top-1/2 -translate-y-1/2 w-10 h-10 rounded-xl bg-slate-900 text-white flex items-center justify-center shadow-sm hover:bg-slate-800 hover:scale-105 active:scale-95 transition-all cursor-pointer"
                title="ส่งคำถาม"
              >
        ...
        <section>
          ...
          {(() => {
            const activeTheme: CardThemeId = (user.cardTheme || getPersistedCardTheme(user) || 'obsidian-gold') as CardThemeId;
            const cardThemeCfg = CARD_THEMES[activeTheme] || CARD_THEMES['obsidian-gold'];
            return (
              <div
                onClick={onOpenIdCardModal}
              ...
              >
                ...
                <GuillochePatternSvg themeId={activeTheme} opacity={0.20} />
                <HologramEmblemSvg themeId={activeTheme} />
        ...
        {/* Section 2.5: Student GPA Trend & Academic Trajectory (Recharts / D3) */}
        {(user.role === 'student' || user.role === 'parent') && (
          <section>
            <GpaAnalyticsChart
              user={user}
              variant="compact"
              onOpenDetailedModal={onOpenGpaModal}
            />
          </section>
        )}
        ...
        <section>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
            ...
            <div className="flex items-center gap-2 justify-between sm:justify-end">
              ...
              <button
                onClick={() => onOpenScheduleModal()}
                className="text-[13px] font-semibold text-blue-700 hover:underline cursor-pointer flex items-center gap-0.5 shrink-0"
              >
          ...
          <div className="bg-white rounded-2xl p-4 sm:p-6 shadow-sm border border-slate-200/80 relative overflow-hidden">
            {/* Vertical timeline line */}
            <div className="absolute top-8 bottom-8 left-[65px] sm:left-[75px] w-0.5 bg-slate-200 z-0"></div>
                  ...
                  >
                    ...
                    <div
                      className={`relative w-4 h-4 rounded-full border-2 shrink-0 z-10 transition-all ${
                        isActive
                          ? 'bg-blue-700 border-white ring-4 ring-blue-700/20 scale-125'
                          : 'bg-white border-slate-400 group-hover:border-blue-700'
                      }`}
                    />
                    ...
                    <div
                      className={`flex-1 rounded-2xl p-3.5 sm:p-4 transition-all duration-200 border ${
                        isActive
                          ...
                          : 'bg-slate-50/70 border-slate-200/60 hover:bg-slate-100 hover:border-slate-300'
                      }`}
                    >
                      ...
                      <div className="text-[12px] sm:text-[13px] text-slate-500 flex items-center gap-2 flex-wrap mt-1">
                        <span className="flex items-center gap-1">
                        ...
                        {item.materialsCount ? (
                          <>
                            <span>•</span>
                            <span className="text-blue-700 font-semibold text-[11px] flex items-center gap-0.5">
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            ...
            {onOpenCampusPulse && (
              <button
                ...
                className="text-[12px] font-bold text-blue-700 hover:underline flex items-center gap-1 cursor-pointer group"
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
            ...
            <div
              ...
              className="bg-white rounded-2xl p-4 shadow-sm border border-slate-200/80 flex flex-col justify-between hover:-translate-y-1 hover:shadow-md hover:border-blue-400 transition-all cursor-pointer group relative overflow-hidden"
              title="คลิกเพื่อดูโครงสร้างประชากรนักเรียนและแผนการเรียน"
            >
              <div className="flex justify-between items-start mb-3">
                <div className="w-10 h-10 rounded-xl bg-blue-50 text-blue-700 flex items-center justify-center group-hover:scale-105 group-hover:bg-blue-700 group-hover:text-white transition-all shadow-2xs">
                  <span className="material-symbols-outlined text-[20px]">group</span>
                </div>
                <span className="text-[10px] font-bold text-blue-700 bg-blue-50 px-2 py-0.5 rounded-md opacity-0 group-hover:opacity-100 transition-opacity">
                  ดูสถิติ ➔
                </span>
              </div>
              <div>
                ...
                <div className="text-[22px] sm:text-[26px] text-slate-900 font-bold tracking-tight">
                  {displayStudentCount.toLocaleString()}
                </div>
              </div>
              <div className="mt-2 text-[10.5px] text-slate-500 group-hover:text-blue-700 flex items-center gap-1 font-medium transition-colors">
            ...
            <div
              ...
              className="bg-white rounded-2xl p-4 shadow-sm border border-slate-200/80 flex flex-col justify-between hover:-translate-y-1 hover:shadow-md hover:border-slate-400 transition-all cursor-pointer group relative overflow-hidden"
              title="คลิกเพื่อดูรายชื่ออาจารย์และสถานะ Office Hours"
            >
              <div className="flex justify-between items-start mb-3">
                <div className="w-10 h-10 rounded-xl bg-slate-100 text-slate-800 flex items-center justify-center group-hover:scale-105 group-hover:bg-slate-900 group-hover:text-white transition-all shadow-2xs">
                  <span className="material-symbols-outlined text-[20px]">school</span>
                </div>
                <span className="text-[10px] font-bold text-slate-800 bg-slate-100 px-2 py-0.5 rounded-md opacity-0 group-hover:opacity-100 transition-opacity">
                  ดูคณาจารย์ ➔
                </span>
              </div>
              <div>
                ...
                <div className="text-[22px] sm:text-[26px] text-slate-900 font-bold tracking-tight">
                  {liveTelemetry.totalTeachers}
                </div>
              </div>
              <div className="mt-2 text-[10.5px] text-slate-500 group-hover:text-slate-900 flex items-center gap-1 font-medium transition-colors">
                <span className="material-symbols-outlined text-[13px]">touch_app</span>
                {liveTelemetry.teachersAvailableCount > 0 ? `พร้อมปรึกษา ${liveTelemetry.teachersAvailableCount} ท่าน` : 'ยังไม่มีอาจารย์ออนไลน์'}
              </div>
            </div>
            ...
            <div
              ...
              className="bg-white rounded-2xl p-4 shadow-sm border border-slate-200/80 flex flex-col justify-between hover:-translate-y-1 hover:shadow-md hover:border-emerald-400 transition-all cursor-pointer group relative overflow-hidden"
              title="คลิกเพื่อดูความหนาแน่น Wi-Fi และเครือข่ายแต่ละอาคาร"
            >
              <div className="flex justify-between items-start mb-3">
                <div className="w-10 h-10 rounded-xl bg-emerald-50 text-emerald-700 flex items-center justify-center group-hover:scale-105 group-hover:bg-emerald-700 group-hover:text-white transition-all shadow-2xs">
                  <span className="material-symbols-outlined text-[20px]">wifi</span>
                </div>
                <span className="text-[10px] font-bold text-emerald-800 bg-emerald-100 px-2 py-0.5 rounded-md opacity-0 group-hover:opacity-100 transition-opacity">
                  ดูโซน ➔
                </span>
              </div>
              <div>
                ...
                <div className="text-[22px] sm:text-[26px] text-slate-900 font-bold tracking-tight">
                  {liveTelemetry.onlineNow}
                </div>
              </div>
              <div className="mt-2 text-[10.5px] text-slate-500 group-hover:text-emerald-800 flex items-center gap-1 font-medium transition-colors">
                <span className="material-symbols-outlined text-[13px]">touch_app</span>
                {liveTelemetry.onlineNow > 0 ? 'ทราฟฟิกเว็บแอป Real-time' : 'ยังไม่มีผู้ใช้งานออนไลน์'}
              </div>
            </div>
            ...
            <div
              ...
              className="bg-white rounded-2xl p-4 shadow-sm border border-slate-200/80 flex flex-col justify-between hover:-translate-y-1 hover:shadow-md hover:border-amber-400 transition-all cursor-pointer group relative overflow-hidden"
              title="คลิกเพื่อดูสถิติการเข้าเรียนและการตรงต่อเวลาแยกตามห้อง"
            >
              <div className="flex justify-between items-start mb-3">
                <div className="w-10 h-10 rounded-xl bg-amber-50 text-amber-700 flex items-center justify-center group-hover:scale-105 group-hover:bg-amber-700 group-hover:text-white transition-all shadow-2xs">
                  <span className="material-symbols-outlined text-[20px]">fact_check</span>
                </div>
                <span className="text-[10px] font-bold text-amber-800 bg-amber-100 px-2 py-0.5 rounded-md opacity-0 group-hover:opacity-100 transition-opacity">
                  ดูรายห้อง ➔
                </span>
              </div>
              <div>
                ...
                <div className="text-[22px] sm:text-[26px] text-slate-900 font-bold tracking-tight">
                  {liveTelemetry.attendanceRate}<span className="text-sm font-normal text-slate-500">%</span>
                </div>
              </div>
              <div className="mt-2 text-[10.5px] text-slate-500 group-hover:text-amber-800 flex items-center gap-1 font-medium transition-colors">
                <span className="material-symbols-outlined text-[13px]">touch_app</span>
                เช็กชื่อ {liveTelemetry.presentCount + liveTelemetry.lateCount} คน / ขาด {liveTelemetry.absentCount} คน
              </div>
            </div>
          </div>
        </section>

        {/* Section 5: Learning Analytics Preview Banner (for Student) */}
        {user.role === 'student' && (
          <section className="bg-slate-900 rounded-3xl p-5 sm:p-6 text-white shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4 border border-slate-800">
            <div className="flex items-center gap-3.5">
              <div className="w-12 h-12 rounded-2xl bg-white/10 backdrop-blur-md flex items-center justify-center text-blue-300 border border-white/15 shrink-0">
                <span className="material-symbols-outlined text-[26px]">monitoring</span>
              </div>
              <div>
                <div className="flex items-center gap-2">
                  <h4 className="font-bold text-base sm:text-lg text-white">
                    ศูนย์วิเคราะห์ผลการเรียนรู้ & การเข้าเรียน 30 วัน (Learning Analytics)
                  </h4>
                ...
                <p className="text-xs text-slate-300 mt-0.5">
                  ตรวจสอบแผนภูมิวงกลมและแท่งเปรียบเทียบคะแนนเฉลี่ยรายวิชา พร้อมแนวโน้มสถิติสแกน RFID ย้อนหลัง 30 วัน
                </p>
              </div>
            </div>
            ...
            <button
              ...
              className="px-4 py-2.5 rounded-xl bg-white text-slate-900 hover:bg-slate-100 font-bold text-xs sm:text-sm shadow-sm transition-all active:scale-95 flex items-center justify-center gap-1.5 shrink-0 cursor-pointer"
            >
              <span>เปิดดูกราฟวิเคราะห์เต็มรูปแบบ</span>
              <span className="material-symbols-outlined text-[16px]">arrow_forward</span>
            </button>
          </section>
        )}
