...
export const AssignmentsView: React.FC<AssignmentsViewProps> = ({
  ...
  const getGitHubSyncStatusBadge = (as: Assignment) => {
    const hasLastSynced = Boolean(as.lastSyncedAt);
    const hasRepoUrl = Boolean(as.githubRepoUrl && as.githubRepoUrl.trim());

    // 1. Green checkmark if lastSyncedAt is present
    if (hasLastSynced) {
      const formattedTime = as.lastSyncedAt
        ? new Date(as.lastSyncedAt).toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit' })
        : '';
      const formattedDate = as.lastSyncedAt
        ? new Date(as.lastSyncedAt).toLocaleDateString('th-TH')
        : '';

      return (
        <div
          className="flex items-center gap-1.5 text-emerald-800 bg-emerald-50 px-2.5 py-1 rounded-lg shrink-0 border border-emerald-200 text-xs font-semibold shadow-2xs"
          title={`GitHub Sync Status: ซิงค์ล่าสุดเรียบร้อยแล้ว (${formattedDate} ${formattedTime} น.)${as.lastCommitSha ? ` [Commit: ${as.lastCommitSha.substring(0, 7)}]` : ''}`}
        >
          <span className="material-symbols-outlined text-[15px] text-emerald-600 font-bold fill-1">
            check_circle
          </span>
          <span className="whitespace-nowrap hidden sm:inline text-[11px]">GitHub Synced</span>
        </div>
      );
    }

    // 2. Warning icon if repository URL is missing
    if (!hasRepoUrl) {
      return (
        <div
          className="flex items-center gap-1.5 text-amber-800 bg-amber-50 px-2.5 py-1 rounded-lg shrink-0 border border-amber-200 text-xs font-semibold shadow-2xs"
          title="GitHub Sync Status: ยังไม่ได้ระบุ Repository URL (Warning)"
        >
          <span className="material-symbols-outlined text-[15px] text-amber-600 font-bold fill-1">
            warning
          </span>
          <span className="whitespace-nowrap hidden sm:inline text-[11px]">No Repo</span>
        </div>
      );
    }

    // 3. Fallback if repo URL is present but pending initial sync
    return (
      <div
        className="flex items-center gap-1.5 text-blue-800 bg-blue-50 px-2.5 py-1 rounded-lg shrink-0 border border-blue-200 text-xs font-semibold shadow-2xs"
        title={`GitHub Sync Status: เชื่อมโยง Repo แล้ว (${as.githubRepoUrl})`}
      >
        <span className="material-symbols-outlined text-[15px] text-blue-600">
          code
        </span>
        <span className="whitespace-nowrap hidden sm:inline text-[11px]">Repo Linked</span>
      </div>
    );
  };
            ...
            filteredAssignments.map((as) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Header Row: Subject, Title, Status & GitHub Sync Status */}
                  <div className="flex justify-between items-start gap-3">
                    ...
                    {/* Top-Right: GitHub Sync Status Indicator & Assignment Status */}
                    <div className="flex items-center gap-1.5 sm:gap-2 shrink-0 self-start">
                      {getGitHubSyncStatusBadge(as)}
