  ...
  const matchAndSetProfile = useCallback((id: string, fallbackName?: string, originalText?: string) => {
    ...
    if (matchedAccount && matchedAccount.user) {
      ...
      if (u.role === 'student') {
        ...
        const liveTimeStr = new Date(scanTs).toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit', second: '2-digit' }) + ' น.';
        pushRealtimeNotification({
          title: `🚌 แจ้งเตือนการแตะบัตรเข้าโรงเรียน: ${u.thaiName}`,
          message: `นักเรียนได้แตะบัตร Smart Digital ID / QR เข้าโรงเรียนเรียบร้อย (เวลาจริง: ${liveTimeStr}) สถานะ: เข้าโรงเรียนปกติ`,
          type: 'attendance',
          priority: 'normal',
          role: 'parent',
          icon: 'how_to_reg',
