...
export const DigitalIdModal: React.FC<DigitalIdModalProps> = ({
  ...
  const [selectedTheme, setSelectedTheme] = useState<CardThemeId>(() => {
    return user.cardTheme || getPersistedCardTheme(user) || 'obsidian-gold';
  });
  const [currentDateTime, setCurrentDateTime] = useState<Date>(new Date());
  const [nfcBeep, setNfcBeep] = useState(false);
  const [toastMessage, setToastMessage] = useState<string | null>(null);

  // Sync selected theme whenever user or modal open state changes
  useEffect(() => {
    if (isOpen && user) {
      const persisted = getPersistedCardTheme(user);
      const activeTheme = user.cardTheme || persisted || 'obsidian-gold';
      setSelectedTheme(activeTheme);
    }
  }, [isOpen, user?.cardTheme, user?.id, user?.studentId, user?.role]);
  ...
  const handleSelectTheme = (themeId: CardThemeId) => {
    ...
    savePersistedCardTheme(user, themeId);
    if (onUpdateTheme) {
    ...
    setToastMessage(`บันทึกลายบัตรถาวร: ${CARD_THEMES[themeId].thaiName}`);
  ...
  return (
    ...
    >
      {/* Toast Notification */}
      {toastMessage && (
        <div className="fixed top-4 right-3 sm:right-6 z-[100] bg-[#121b2e] text-white px-4 py-2 rounded-2xl shadow-2xl text-xs font-semibold flex items-center gap-2 border border-amber-400/40 animate-slideInRightToast max-w-[calc(100vw-24px)] sm:max-w-md pointer-events-auto">
          <span className="material-symbols-outlined text-amber-400 text-[18px] shrink-0">palette</span>
