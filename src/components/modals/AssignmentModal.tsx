...
import { Assignment, AssignmentAttachment, GithubCommitInfo } from '../../types';
    ...
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
      <div className="bg-white rounded-[28px] max-w-2xl w-full shadow-2xl overflow-hidden border border-slate-100 flex flex-col max-h-[92vh] animate-scaleIn">
        ...
        <form onSubmit={handleSubmit} className="p-5 sm:p-6 overflow-y-auto flex-1 flex flex-col gap-5">
          ...
          {/* Linked GitHub Repository & Commit Sync */}
          <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 flex flex-col gap-3">
            <div className="flex items-center justify-between flex-wrap gap-2">
              <label className="text-xs font-bold text-slate-800 uppercase tracking-wider flex items-center gap-1.5">
                <span className="material-symbols-outlined text-[17px] text-[#1550d3]">code</span>
                <span>เชื่อมโยง GitHub Repository (Git Commit Sync)</span>
              </label>
              <span className="text-[10px] font-bold text-slate-500 bg-white px-2 py-0.5 rounded-md border border-slate-200">
                Optional / ตรวจสอบคอมมิตอัตโนมัติ
              </span>
            </div>
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                <input
                  ...
                  placeholder="เช่น https://github.com/facebook/react หรือ owner/repo"
                  className="w-full pl-3 pr-3 py-2.5 rounded-xl border border-slate-200 text-xs sm:text-sm bg-white font-mono focus:border-[#1550d3] focus:ring-2 focus:ring-[#1550d3]/20 focus:outline-none placeholder:text-slate-400"
                />
              </div>
              <button
                type="button"
                onClick={handleManualGithubCheck}
                disabled={isCheckingGithub || !githubRepoUrl.trim()}
                className="px-3.5 py-2.5 bg-[#121b2e] hover:bg-[#1e293b] disabled:opacity-40 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer shrink-0 shadow-xs"
                title="ตรวจสอบคอมมิตล่าสุดจาก GitHub ทันที"
              >
                <span className={`material-symbols-outlined text-[16px] ${isCheckingGithub ? 'animate-spin' : ''}`}>
                  {isCheckingGithub ? 'sync' : 'search'}
                </span>
                <span className="hidden sm:inline">
                  {isCheckingGithub ? 'กำลังตรวจสอบ...' : 'ตรวจสอบคอมมิต'}
                </span>
              </button>
            </div>
            ...
            {githubSyncMessage && (
              ...
              >
                <span className="material-symbols-outlined text-[16px]">
                  {githubSyncMessage.type === 'success' ? 'check_circle' : 'error'}
                </span>
                <span>{githubSyncMessage.text}</span>
              </div>
            )}

            {/* Latest Commit Details Card */}
            {latestCommit && (
              <div className="p-3 rounded-xl bg-white border border-slate-200/90 shadow-2xs flex flex-col gap-1.5 text-xs">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="px-2 py-0.5 rounded-md bg-purple-50 text-purple-700 font-mono font-bold text-[11px] border border-purple-200">
                      Commit: {latestCommit.shortSha}
                    </span>
                    <span className="text-[11px] text-slate-500 font-medium truncate max-w-[160px]">
                      โดย {latestCommit.authorName}
                    </span>
                  </div>
                  {latestCommit.url && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-[11px] text-[#1550d3] hover:underline font-bold flex items-center gap-0.5"
                    >
                ...
                <div className="text-[10px] text-slate-400">
                  ระบบจะตรวจสอบคอมมิตใหม่และส่งการแจ้งเตือน Push Notification อัตโนมัติเมื่อกดส่งงาน
                </div>
              </div>
            )}
          </div>
