...
import { getStoredAccounts } from '../../services/firebaseService';
      ...
      <div className="bg-white rounded-[28px] max-w-3xl w-full shadow-2xl overflow-hidden border border-slate-100 flex flex-col max-h-[92vh] animate-scaleIn">
        ...
        <div className="flex items-center gap-1.5 px-4 pt-3 pb-2 bg-[#f4f6fb] border-b border-slate-200 overflow-x-auto no-scrollbar">
          ...
          >
            ...
            <span>นักเรียน ({liveTelemetry.totalStudents.toLocaleString()})</span>
          </button>
          ...
          >
            ...
            <span>คณาจารย์ ({liveTelemetry.totalTeachers})</span>
          </button>
          ...
          >
            ...
            <span>ออนไลน์ ({liveTelemetry.onlineNow})</span>
          </button>
          ...
          >
            ...
            <span>การเข้าเรียน ({liveTelemetry.attendanceRate}%)</span>
          </button>
        </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                >
                  ...
                  <div>
                    ...
                    <span className="text-2xl font-bold text-[#121b2e]">{liveTelemetry.totalStudents.toLocaleString()}</span>
                  </div>
                  <div className="mt-2 text-[11px] text-[#00694d] font-semibold flex items-center gap-1">
                    <span className="material-symbols-outlined text-[14px]">trending_up</span>
                    {liveTelemetry.totalStudents > 0 ? 'Active System Accounts' : 'ไม่มีข้อมูลนักเรียน (0)'}
                  </div>
                </div>
                ...
                >
                  ...
                  <div>
                    ...
                    <span className="text-2xl font-bold text-[#121b2e]">{liveTelemetry.totalTeachers}</span>
                  </div>
                  <div className="mt-2 text-[11px] text-[#5f3add] font-semibold flex items-center gap-1">
                    <span className="material-symbols-outlined text-[14px]">check_circle</span>
                    {liveTelemetry.totalTeachers > 0 ? `พร้อมปรึกษา ${liveTelemetry.teachersAvailableCount} ท่าน` : 'ไม่มีอาจารย์ในระบบ (0)'}
                  </div>
                </div>
                ...
                >
                  ...
                  <div>
                    ...
                    <span className="text-2xl font-bold text-[#121b2e]">{liveTelemetry.onlineNow}</span>
                  </div>
                  <div className="mt-2 text-[11px] text-[#00694d] font-semibold flex items-center gap-1">
                    <span className="w-1.5 h-1.5 rounded-full bg-[#20C997] animate-pulse" />
                    {liveTelemetry.onlineNow > 0 ? 'Active Session Live' : 'ไม่มีผู้ใช้ออนไลน์ (0)'}
                  </div>
                </div>
                ...
                >
                  ...
                  <div>
                    ...
                    <span className="text-2xl font-bold text-[#121b2e]">{liveTelemetry.attendanceRate}%</span>
                  </div>
                  <div className="mt-2 text-[11px] text-amber-800 font-semibold flex items-center gap-1">
                    <span className="material-symbols-outlined text-[14px]">military_tech</span>
                    {liveTelemetry.attendanceRate > 0 ? `มาเรียน ${liveTelemetry.presentCount + liveTelemetry.lateCount} คน / ขาด ${liveTelemetry.absentCount}` : 'ยังไม่มีข้อมูลเช็กชื่อ (0%)'}
                  </div>
                </div>
              </div>
          ...
          {activeTab === 'students' && (
            <div className="space-y-5 animate-fadeIn">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs">
                <div>
                  ...
                  <p className="text-xs text-[#737686]">
                    จำนวนนักเรียนทั้งหมด {liveTelemetry.totalStudents.toLocaleString()} คน {liveTelemetry.totalStudents === 0 ? '(ยังไม่มีบัญชีนักเรียนลงทะเบียนในระบบ = 0)' : 'แบ่งตามระดับชั้นและแผนการเรียน'}
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <select
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {liveTelemetry.studentsByGrade
                  .filter((item) => {
                    if (studentGradeFilter === 'all') return true;
                    if (studentGradeFilter === 'm3') {
                      return item.gradeKey === 'm1' || item.gradeKey === 'm2' || item.gradeKey === 'm3';
                    }
                    return item.gradeKey === studentGradeFilter;
                  })
                  .map((item, idx) => (
              ...
              <div className="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200/80 shadow-xs">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
                  <div>
                    ...
                    <p className="text-xs text-[#737686]">
                      จำนวนคณาจารย์ในระบบ {liveTelemetry.totalTeachers} ท่าน ประจำกลุ่มสาระการเรียนรู้ {liveTelemetry.totalTeachers === 0 ? '(ยังไม่มีบัญชีอาจารย์ลงทะเบียนในระบบ = 0)' : ''}
                    </p>
                  </div>
                  <span className="px-2.5 py-1 bg-purple-50 text-[#5f3add] border border-purple-200 rounded-xl text-xs font-bold shrink-0 w-fit">
                    {liveTelemetry.teachersAvailableCount > 0 ? `พร้อมปรึกษาขณะนี้ ${liveTelemetry.teachersAvailableCount} ท่าน` : 'ยังไม่มีอาจารย์ออนไลน์'}
                  </span>
                </div>

                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 pt-1">
                  {liveTelemetry.teachersByDept.map((d, i) => (
            ...
            <div className="space-y-5 animate-fadeIn">
                ...
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <h3 className="font-bold text-base text-[#121b2e] flex items-center gap-2">
                      ...
                      {liveTelemetry.onlineNow} บัญชีผู้ใช้ที่กำลังออนไลน์ในระบบ Real-Time
                    </h3>
                    <p className="text-xs text-[#737686]">
                      ความหนาแน่นของผู้ใช้งานเครือข่ายและระบบคลาวด์โรงเรียน School Nexus
                    </p>
                  </div>
                  <span className="px-3 py-1 bg-emerald-50 text-[#00694d] border border-emerald-200 rounded-xl text-xs font-bold">
                    Cloud Presence: {liveTelemetry.onlineNow > 0 ? 'Active' : 'Idle (0)'}
                  </span>
                </div>

                {/* Zone Traffic Bars */}
                <div className="space-y-3.5">
                  {liveTelemetry.zonesOnline.map((zone, idx) => {
                      ...
                      <div key={idx} className="p-3 rounded-xl bg-[#f8f9fe] border border-slate-200/60">
                        <div className="flex justify-between items-center text-xs mb-1.5">
                          ...
                          <span className="font-semibold text-[#00694d]">
                            {zone.users} ผู้ใช้ ({ratio}%) • Ping {zone.ping}
                          </span>
                        </div>
                        <div className="w-full bg-slate-200 h-2 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${Math.min(100, Math.max(zone.users > 0 ? 5 : 0, ratio))}%`,
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="bg-white p-4 rounded-2xl border border-slate-200/80 text-center shadow-xs">
                  ...
                  <div className="text-xl font-bold text-[#121b2e] mt-1">{liveTelemetry.devicesBreakdown.laptops} เครื่อง</div>
                  <div className="text-xs text-[#737686]">คอมพิวเตอร์และแล็ปท็อป (Desktop/Web)</div>
                </div>

                <div className="bg-white p-4 rounded-2xl border border-slate-200/80 text-center shadow-xs">
                  <span className="material-symbols-outlined text-[28px] text-[#5f3add]">tablet_mac</span>
                  <div className="text-xl font-bold text-[#121b2e] mt-1">{liveTelemetry.devicesBreakdown.tablets} เครื่อง</div>
                  <div className="text-xs text-[#737686]">แท็บเล็ตและอุปกรณ์พกพา</div>
                </div>

                <div className="bg-white p-4 rounded-2xl border border-slate-200/80 text-center shadow-xs">
                  <span className="material-symbols-outlined text-[28px] text-[#20C997]">smartphone</span>
                  <div className="text-xl font-bold text-[#121b2e] mt-1">{liveTelemetry.devicesBreakdown.smartphones} เครื่อง</div>
                  <div className="text-xs text-[#737686]">สมาร์ตโฟนผ่าน Nexus Web/PWA</div>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="space-y-5 animate-fadeIn">
              <div className="bg-gradient-to-br from-[#1550d3] to-[#3c6bed] text-white p-5 rounded-2xl shadow-md flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                  ...
                  <div className="text-3xl sm:text-4xl font-bold mt-1">{liveTelemetry.attendanceRate}%</div>
                  <p className="text-xs text-[#b5c4ff] mt-1">
                    {liveTelemetry.totalAttendanceRecords > 0
                      ? `เข้าเรียนตรงเวลา ${liveTelemetry.presentCount} คน จากการบันทึกทั้งหมด ${liveTelemetry.totalAttendanceRecords} ครั้ง`
                      : 'ยังไม่มีประวัติการเช็กชื่อในระบบวันนี้ (ค่าเริ่มต้น = 0)'}
                  </p>
                </div>
                <div className="flex gap-2">
                  <div className="px-3.5 py-2 rounded-xl bg-white/20 backdrop-blur-md text-center">
                    ...
                    <div className="text-lg font-bold">{liveTelemetry.leaveCount} คน</div>
                  </div>
                  <div className="px-3.5 py-2 rounded-xl bg-white/20 backdrop-blur-md text-center">
                    <div className="text-xs text-white/80">สาย (&gt;08:00)</div>
                    <div className="text-lg font-bold">{liveTelemetry.lateCount} คน</div>
                  </div>
                  <div className="px-3.5 py-2 rounded-xl bg-white/20 backdrop-blur-md text-center">
                    <div className="text-xs text-white/80">ขาดเรียน</div>
                    <div className="text-lg font-bold">{liveTelemetry.absentCount} คน</div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs">
                <h4 className="font-bold text-sm text-[#121b2e] mb-3 flex items-center justify-between">
                  ...
                  <span className="text-xs font-normal text-[#737686]">ประมวลผลผ่าน Gate Scanner RFID / Attendance Service</span>
                </h4>

                <div className="space-y-2.5">
                  {liveTelemetry.attendanceByGrade.map((item) => (
                    <div
                      key={item.grade}
