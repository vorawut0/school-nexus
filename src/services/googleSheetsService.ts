...
export function classifyGoogleSheetError(
  ...
  // 1.5. User Cancelled / Popup Closed (auth/popup-closed-by-user)
  if (
    ...
    return {
      ...
      title: 'การเข้าสู่ระบบถูกยกเลิก (Sign-in Cancelled)',
      message: 'หน้าต่างเข้าสู่ระบบ Google ถูกปิดก่อนทำรายการเสร็จ คุณสามารถกดปุ่มเชื่อมต่อใหม่อีกครั้งเมื่อพร้อม',
      statusCode: 200,
      technicalDetails: rawMsg,
      spreadsheetId: cleanId,
      spreadsheetUrl: sheetUrl,
      actionableSteps: [
        'คลิกปุ่ม "เชื่อมต่อ Google (Sign in with Google)" อีกครั้ง',
        'เลือกบัญชี Google และอนุญาตสิทธิ์การเข้าถึง Google Sheets',
      ],
      suggestedAction: {
        label: 'เชื่อมต่อบัญชี Google ใหม่',
        icon: 'account_circle',
        actionType: 'sign_in',
      },
    };
...
export async function signInWithGoogleSheets(): Promise<{
  ...
  try {
    ...
    if (
      ...
      console.log('[GoogleSheetsService] Sign-in popup closed by user.');
    } else {
...
/**
 * Download student grades as a CSV/Excel file directly
 */
export function downloadGradesAsCSV(
  assignmentTitle: string,
  subjectCode: string,
  submissions: {
    studentId: string;
  ...
  const headers = ['ลำดับ', 'รหัสนักเรียน', 'ชื่อ-นามสกุล', 'คะแนนที่ได้', 'คะแนนเต็ม', 'สถานะ', 'ข้อเสนอแนะอาจารย์', 'เวลาที่ส่งงาน'];
  ...
  const avg = submissions.length > 0 ? (totalScore / submissions.length).toFixed(2) : '0';
  ...
  const csvContent = '\uFEFF' + [
    ...
    ...rows.map((r) => r.join(',')),
    '',
    `"คะแนนเฉลี่ยทั้งห้อง",,"${avg} / ${submissions[0]?.maxScore || 100}","จำนวนนักเรียน ${submissions.length} คน"`,
  ].join('\r\n');
  ...
  a.href = url;
  a.download = `คะแนน_${subjectCode || 'Gradebook'}_${assignmentTitle.slice(0, 15)}.csv`;
...
export async function exportFirestoreGradesToGoogleSheet(
  ...
  // If not authenticated, trigger interactive Google sign-in
  const token = await getGoogleAccessToken();
  if (!token) {
...
/**
 * Download attendance records as CSV directly
 */
export function downloadAttendanceAsCSV(
  ...
  const headers = ['ลำดับ', 'รหัสนักเรียน', 'ชื่อ-นามสกุล', 'สถานะการเข้าเรียน', 'เวลาเช็กชื่อ', 'หมายเหตุ'];
  let presentCount = 0;
  let lateCount = 0;
  let absentCount = 0;
  let leaveCount = 0;

  const rows = attendanceList.map((att, idx) => {
    let statusText = 'มาเรียน (Present)';
    if (att.status === 'present') presentCount++;
    else if (att.status === 'late') { lateCount++; statusText = 'มาสาย (Late)'; }
    else if (att.status === 'absent') { absentCount++; statusText = 'ขาดเรียน (Absent)'; }
    else if (att.status === 'leave') { leaveCount++; statusText = 'ลา (Leave)'; }
  ...
  const csvContent = '\uFEFF' + [
    ...
    `"วันที่: ${dateLabel}",,"สร้างเมื่อ: ${new Date().toLocaleString('th-TH')}"`,
    '',
    headers.join(','),
    ...rows.map((r) => r.join(',')),
    '',
    `"สรุปยอดรวม:",,"มา: ${presentCount} คน","สาย: ${lateCount} คน","ลา: ${leaveCount} คน","ขาด: ${absentCount} คน (รวม ${attendanceList.length} คน)"`,
  ].join('\r\n');
  ...
  const a = document.createElement('a');
  a.href = url;
  a.download = `เช็กชื่อ_${classNameLabel}_${dateLabel.replace(/\//g, '-')}.csv`;
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
  URL.revokeObjectURL(url);
