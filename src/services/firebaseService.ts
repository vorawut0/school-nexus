...
// Universal avatar persistence lookup to ensure custom photos are synchronized across all platforms and roles
  ...
  try {
    ...
    if (typeof userOrKey === 'string') {
      ...
      return (
        ...
        localStorage.getItem('sn_global_avatar') ||
        null
      );
    }
    ...
    // 2. Specific role + email combo (ensures same email on different roles has separate avatars if needed)
    ...
    // 4. Email-level fallback across roles for the same user
    if (user.email) {
      const fromEmail = localStorage.getItem(`sn_avatar_email_${user.email.toLowerCase()}`);
    ...
    // 6. Global custom avatar fallback
    const globalAvatar = localStorage.getItem('sn_global_avatar');
...
// Universal card theme persistence lookup to ensure custom card theme colors never revert
export function getPersistedCardTheme(userOrKey: UserProfile | string | null | undefined): UserProfile['cardTheme'] | null {
  try {
    if (!userOrKey) return null;
    const VALID_THEMES: Array<NonNullable<UserProfile['cardTheme']>> = [
      'obsidian-gold',
      'cyber-blue',
      'emerald-tech',
      'aurora-violet',
    ...
    if (typeof userOrKey === 'string') {
      const lower = userOrKey.toLowerCase();
      const val = (
        localStorage.getItem(`sn_card_theme_${userOrKey}`) ||
        localStorage.getItem(`sn_card_theme_${lower}`) ||
        localStorage.getItem(`sn_card_theme_role_${lower}`) ||
        localStorage.getItem('sn_global_card_theme')
      ) as UserProfile['cardTheme'] | null;
      if (val && VALID_THEMES.includes(val)) return val;
      return null;
    }

    const user = userOrKey;

    // 1. Specific user ID first
    if (user.id) {
      const fromId = localStorage.getItem(`sn_card_theme_${user.id}`) as UserProfile['cardTheme'] | null;
      if (fromId && VALID_THEMES.includes(fromId)) return fromId;
    }

    // 2. Specific role + email combo
    if (user.role && user.email) {
      const fromRoleEmail = localStorage.getItem(`sn_card_theme_${user.role}_${user.email.toLowerCase()}`) as UserProfile['cardTheme'] | null;
      if (fromRoleEmail && VALID_THEMES.includes(fromRoleEmail)) return fromRoleEmail;
    }

    // 3. Specific role + studentId combo
    if (user.role && user.studentId) {
      const fromRoleStudentId = localStorage.getItem(`sn_card_theme_${user.role}_${user.studentId.toLowerCase()}`) as UserProfile['cardTheme'] | null;
      if (fromRoleStudentId && VALID_THEMES.includes(fromRoleStudentId)) return fromRoleStudentId;
    }

    // 4. Role-specific fallback
    if (user.role) {
      const fromRole = localStorage.getItem(`sn_card_theme_role_${user.role}`) as UserProfile['cardTheme'] | null;
      if (fromRole && VALID_THEMES.includes(fromRole)) return fromRole;
    }

    // 5. Global fallback
    const globalTheme = localStorage.getItem('sn_global_card_theme') as UserProfile['cardTheme'] | null;
    if (globalTheme && VALID_THEMES.includes(globalTheme)) return globalTheme;
  } catch {
    // ignore
  }
  return null;
...
export function savePersistedCardTheme(user: UserProfile | string, theme: UserProfile['cardTheme']): void {
  if (!theme) return;
  try {
    localStorage.setItem('sn_global_card_theme', theme);
    if (typeof user === 'string') {
      localStorage.setItem(`sn_card_theme_${user}`, theme);
      return;
    }
    if (user.id) {
      localStorage.setItem(`sn_card_theme_${user.id}`, theme);
    }
    if (user.role) {
      localStorage.setItem(`sn_card_theme_role_${user.role}`, theme);
      if (user.email) {
        localStorage.setItem(`sn_card_theme_${user.role}_${user.email.toLowerCase()}`, theme);
      }
      if (user.studentId) {
        localStorage.setItem(`sn_card_theme_${user.role}_${user.studentId.toLowerCase()}`, theme);
      }
    }
  } catch (e) {
    console.warn('Failed to store card theme key in localStorage:', e);
  }
}
...
export function savePersistedAvatar(user: UserProfile): void {
  ...
  try {
    // 1. Save global avatar fallback
    localStorage.setItem('sn_global_avatar', user.avatar);

    // 2. Save strictly under this specific user ID
    if (user.id) {
      localStorage.setItem(`sn_avatar_${user.id}`, user.avatar);
    }
    // 3. Save by email so same account across roles / devices gets synced
    if (user.email) {
      localStorage.setItem(`sn_avatar_email_${user.email.toLowerCase()}`, user.avatar);
    }
    // 4. Save scoped by role
    if (user.role) {
    ...
    // 5. Fire global storage event for cross-tab or cross-platform live update
    window.dispatchEvent(
      new CustomEvent('sn_avatar_changed', {
        detail: {
          avatar: user.avatar,
          userId: user.id,
          email: user.email,
...
export async function saveUserProfile(userProfile: UserProfile): Promise<void> {
  ...
  // 1.1 Save Card Theme in multi-key lookup to guarantee no revert
...
export function getDefaultSeedAccounts(): StoredAccountRecord[] {
  return [
    ...
    },
    {
      ...
      email: DEMO_PRESET_USERS.admin?.email || 'vorawutphetrai17@gmail.com',
      name: DEMO_PRESET_USERS.admin?.name || 'ADMINISTRATOR SYSTEM',
      thaiName: DEMO_PRESET_USERS.admin?.thaiName || 'ผู้ดูแลระบบไอทีและแคมปัส (วรวุฒิ เพ็ชรราย)',
      role: 'admin',
      password: 'password',
...
// Safe no-op to ensure accounts are never lost
export async function purgeAccountImmediately(email: string = 'vorawutphetrai17@gmail.com'): Promise<void> {
  // Safe no-op to ensure accounts are never unintentionally deleted
  console.debug('purgeAccountImmediately invoked safely (no-op):', email);
}
...
export async function registerNewUser(data: RegisterUserData): Promise<{ success: boolean; user?: UserProfile; error?: string }> {
  try {
    ...
    // 0. Validate real email structure
    if (trimmedEmail) {
      const validation = validateRealEmail(trimmedEmail);
      if (!validation.isValid) {
        return {
          success: false,
          error: validation.error || 'อีเมลที่ใช้ต้องเป็นอีเมลที่มีอยู่จริง เช่น yourname@gmail.com',
        };
      }
    }
    
    // Enforce admin role restriction: only the designated admin email can register as admin
    if (data.role === 'admin' && trimmedEmail.toLowerCase() !== SUPER_ADMIN_EMAIL.toLowerCase()) {
      return {
        success: false,
...
// User Sign In Service with Instant Local-First Verification & Fast Cloud Fallback
export async function signInUser(
  ...
  try {
    ...
    // Helper timeout wrapper to ensure cloud operations NEVER hang the UI
    const withTimeout = <T>(promise: Promise<T>, ms: number = 2000): Promise<T | null> => {
      let timeoutId: any;
    ...
    const isAnyDemoPassword =
      inputPassword === '123456' ||
      inputPassword === 'password' ||
      inputPassword === 'demo1234' ||
      inputPassword === 'nexus2026' ||
      inputPassword === 'admin1234' ||
      inputPassword === 'admin' ||
      inputPassword === 'school2026';

    // =========================================================================
    // STEP 1: INSTANT LOCAL REGISTERED ACCOUNTS LOOKUP (< 5ms response)
    // =========================================================================
    const localAccounts = getStoredAccounts();
    const matchingLocalList = localAccounts.filter((a) => {
      const aId = a.id?.toLowerCase();
      const aStudentId = a.studentId?.toLowerCase();
      const aEmail = a.email?.toLowerCase();
      const aName = a.name?.toLowerCase();
      const aThaiName = a.thaiName?.toLowerCase();
      return (
        aId === lowerId ||
        aStudentId === lowerId ||
        aEmail === lowerId ||
    ...
    if (matchingLocalList.length > 0) {
      // Prioritize account matching selectedRole if available, otherwise take the matched account
    ...
    const matchedSeed = seedAccounts.find((s) => {
      ...
      const sStudentId = s.studentId?.toLowerCase();
    ...
    // =========================================================================
    // STEP 3: FIREBASE AUTH DIRECT VERIFY (If Email & Password)
    // =========================================================================
    ...
    // If account was matched locally but password failed
    if (matchingLocalList.length > 0) {
      return {
        success: false,
        error: 'รหัสผ่านไม่ถูกต้อง กรุณาตรวจสอบรหัสผ่านของคุณอีกครั้ง',
    ...
    return {
      success: false,
      error: 'ไม่พบบัญชีผู้ใช้งานนี้ในระบบ กรุณาตรวจสอบรหัสประจำตัว หรือกดแท็บ "ลงทะเบียนบัญชีใหม่"',
    };
  } catch (error: any) {
...
export async function checkGoogleEmailRegistered(email: string, uid?: string): Promise<{
  ...
  try {
    ...
    if (!qSnapExact.empty) {
      ...
      const customTheme = getPersistedCardTheme(userProfile);
      if (customTheme) userProfile.cardTheme = customTheme;
    ...
    if (matchedLocal) {
      const customTheme = getPersistedCardTheme(matchedLocal.user);
      if (customTheme) matchedLocal.user.cardTheme = customTheme;
...
export function formatRealtimeNotificationTime(timestamp?: number, fallbackTime?: string): string {
  if (!timestamp) return fallbackTime || 'เมื่อสักครู่';
  const now = Date.now();
  ...
  const exactTimeStr = notifDate.toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit', second: '2-digit' }) + ' น.';

  if (diffSec < 15) {
    return `เมื่อสักครู่ (${exactTimeStr})`;
  }
  if (diffSec < 60) {
    return `${diffSec} วินาทีที่แล้ว (${exactTimeStr})`;
  }
  if (diffMin < 60) {
    return `${diffMin} นาทีที่แล้ว (${exactTimeStr})`;
  }
  if (diffHours < 24) {
    return `วันนี้ ${exactTimeStr}`;
  }
  const dateStr = notifDate.toLocaleDateString('th-TH', { day: 'numeric', month: 'short' });
  return `${dateStr} ${exactTimeStr}`;
}
...
export async function pushRealtimeNotification(data: {
  ...
  const exactTimeStr = new Date(ts).toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit', second: '2-digit' }) + ' น.';

  const newNotif: NotificationItem = {
    id: `notif-${ts}-${Math.floor(Math.random() * 1000)}`,
    title: data.title,
    message: data.message,
    time: data.time || `เมื่อสักครู่ (${exactTimeStr})`,
    type: data.type,
    read: false,
    role: data.role || 'all',
...
export async function simulateRoleRealtimeNotification(role: UserRole): Promise<NotificationItem> {
  ...
  const nowDate = now.toLocaleDateString('th-TH', { day: 'numeric', month: 'short' });

  if (role === 'teacher') {
    const pool = [
      {
        title: '📝 มีนักเรียนส่งการบ้านใหม่',
        message: `วรวุฒิ เพ็ชรราย (ม.6/1) ส่ง "โครงงานโมเดล Deep Learning จำแนกภาพ CNN" เรียบร้อยแล้ว (เวลาจริง: ${nowTime})`,
      ...
      },
      {
        ...
        message: `ณัฐพล ศิริพันธ์ ขอลาป่วยวิชาช่วงบ่าย วันที่ ${nowDate} ผู้ปกครองยืนยันผ่านระบบแล้ว (เวลาจริง: ${nowTime})`,
    ...
    const pool = [
      {
        ...
        message: `Node 07 (อาคาร 4 ชั้น 2) สัญญาณ RSSI ดรอปเล็กน้อย ระบบกำลังปรับอัตราส่งสัญญาณ (ตรวจพบเวลา: ${nowTime})`,
        type: 'iot' as const,
        priority: 'high' as const,
        icon: 'sensors',
        role: 'admin' as const,
      },
      {
        ...
        message: `อาจารย์ชัญญา ธนะไพศาล สั่งปลดล็อกห้อง Digital Creative Studio สำเร็จ (เวลาจริง: ${nowTime})`,
    ...
    const pool = [
      {
        ...
        message: `แตะบัตร RFID ผ่านประตูหลักโรงเรียนเรียบร้อย (เวลาจริง: ${nowTime}) สถานะ: เข้าโรงเรียนตรงเวลา`,
        type: 'attendance' as const,
        priority: 'normal' as const,
        icon: 'how_to_reg',
      ...
      },
      {
        ...
        message: `ชำระค่าอาหารกลางวัน ฿45.00 ยอดคงเหลือในกระเป๋าบัตร ฿375.00 (ทำรายการเวลาจริง: ${nowTime})`,
  ...
  const pool = [
    {
      ...
      message: `คุณได้รับคะแนน 20/20 เต็ม ในชิ้นงาน "โครงงาน CNN Image Classifier" พร้อม +50 XP! (บันทึกเวลาจริง: ${nowTime})`,
      type: 'grade' as const,
      priority: 'high' as const,
      icon: 'military_tech',
    ...
    },
    {
      ...
      message: `วิชา CS33201 ปัญญาประดิษฐ์ประยุกต์ จะเริ่มในอีก 10 นาที ที่ห้อง LAB-401 (เวลาปัจจุบัน: ${nowTime})`,
      type: 'class' as const,
      priority: 'normal' as const,
      icon: 'schedule',
      role: 'student' as const,
